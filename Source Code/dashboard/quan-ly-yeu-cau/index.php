...
if(isset($_POST['update_status'])) {
    ...
    try {
        ...
        $sql = "INSERT INTO lich_su_cap_nhat (yeu_cau_id, nguoi_cap_nhat, trang_thai_cu, trang_thai_moi, ghi_chu) 
                VALUES (?, ?, (SELECT trang_thai FROM yeu_cau_sua_chua WHERE id = ?), ?, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$yeu_cau_id, $_SESSION['user_id'], $yeu_cau_id, $trang_thai, $ghi_chu]);
...
if($search) {
    $where .= " AND (yc.ma_don LIKE ? OR yc.ho_ten LIKE ? OR yc.so_dien_thoai LIKE ? OR yc.dia_chi LIKE ?)";
    $search_param = "%$search%";
...
// Đếm tổng số bản ghi
$stmt = $pdo->prepare("SELECT COUNT(*) FROM yeu_cau_sua_chua yc $where");
$stmt->execute($params);
$total_records = $stmt->fetchColumn();
$total_pages = ceil($total_records / $limit);

// Query lấy danh sách yêu cầu
$sql = "SELECT yc.*, u.username as nguoi_xu_ly_username, ui.fullname as nguoi_xu_ly_name
...
// Thêm vào phần đầu file để lấy thống kê
$stats = [
        ...
        <div class="card-header bg-white py-3">
            <div class="row align-items-center">
                ...
                <div class="col text-end">
                    <button type="button" class="btn btn-primary" data-bs-toggle="collapse" data-bs-target="#searchForm">
        ...
        <div class="card-body">
                            ...
                            <div class="col-md-2">
                                <select name="status" class="form-select">
                                    ...
                                    <?php foreach($status_text as $key => $text): ?>
                                        <option value="<?php echo $key; ?>" <?php echo $status_filter === (string)$key ? 'selected' : ''; ?>>
                                            <?php echo $text; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if($yc['nguoi_xu_ly']): ?>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <div class="fw-bold"><?php echo $yc['nguoi_xu_ly_name'] ?: $yc['nguoi_xu_ly_username']; ?></div>
                                        </div>
                                    </div>
                                <?php else: ?>
                            ...
                            <td>
                                <div class="btn-group">
                                    <button type="button" class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#viewModal<?php echo $yc['id']; ?>" title="Xem chi tiết">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                    <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#updateModal<?php echo $yc['id']; ?>" title="Cập nhật">
            ...
            <nav aria-label="Page navigation" class="mt-4">
                <ul class="pagination justify-content-center">
                    ...
                    if($start_page > 1): ?>
                        <li class="page-item">
                            <a class="page-link" href="?page=1&search=<?php echo urlencode($search); ?>&status=<?php echo $status_filter; ?>&date_from=<?php echo $date_from; ?>&date_to=<?php echo $date_to; ?>">1</a>
                        </li>
                        <?php if($start_page > 2): ?>
                    ...
                    if($end_page < $total_pages): 
                        ...
                        <?php endif; ?>
                        <li class="page-item">
                            <a class="page-link" href="?page=<?php echo $total_pages; ?>&search=<?php echo urlencode($search); ?>&status=<?php echo $status_filter; ?>&date_from=<?php echo $date_from; ?>&date_to=<?php echo $date_to; ?>">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        <div class="card h-100">
                            ...
                            <div class="card-body">
                                <p><i class="fas fa-user-circle me-2"></i><strong>Họ tên:</strong> <?php echo $yc['ho_ten']; ?></p>
                                <p><i class="fas fa-phone me-2"></i><strong>Số điện thoại:</strong> <?php echo $yc['so_dien_thoai']; ?></p>
                                <p><i class="fas fa-map-marker-alt me-2"></i><strong>Địa chỉ:</strong> <?php echo $yc['dia_chi']; ?></p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                <div class="position-relative">
                                    ...
                                    <?php else: ?>
                                        <video class="img-thumbnail w-100" controls style="height: 150px; object-fit: cover;">
                                            <source src="/assets/media/uploads/yeu-cau-sua-chua/<?php echo $file; ?>" type="video/<?php echo $file_ext; ?>">
                                        </video>
                                    <?php endif; ?>
                                </div>
                            </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#updateModal<?php echo $yc['id']; ?>">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="trang_thai" class="form-select" required>
                            <?php foreach($status_text as $key => $text): ?>
                                <option value="<?php echo $key; ?>" <?php echo $yc['trang_thai'] == $key ? 'selected' : ''; ?>>
                                    <?php echo $text; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="nguoi_xu_ly" class="form-select" <?php echo $_SESSION['role'] > 2 ? 'disabled' : ''; ?>>
                            ...
                            ?>
                                <option value="<?php echo $nv['id']; ?>" <?php echo $yc['nguoi_xu_ly'] == $nv['id'] ? 'selected' : ''; ?>>
